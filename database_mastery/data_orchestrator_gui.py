...
from PySide6.QtCore import Qt, QSize, QRunnable, QThreadPool, Signal, QObject
...
class DatabaseTaskWorker(QRunnable):
    ...
    def boot_docker_node(self):
        ...
        try:
            # Stripped out the -f flag completely to utilize standard docker-compose.yml auto-discovery
            cmd = f"docker compose up -d {service}"
            ...
            if process.returncode == 0:
                ...
                self.signals.log.emit(f"⚠️ Docker Warning: {process.stderr.strip()}")
        except subprocess.TimeoutExpired:
            self.signals.log.emit("❌ Timeout reached trying to invoke Docker Compose process.")
        except Exception as e:
            self.signals.log.emit(f"❌ Failed to run boot command: {str(e)}")
    ...
    def execute_data_fetch(self):
        try:
            ...
            if self.db_name == "PostgreSQL":
                ...
                cursor = client['lab_analytics']['events'].find().limit(10)
                ...
                keys = r.keys("log:*")[:10]
                rows, headers = [[k.decode('utf-8'), r.get(k).decode('utf-8')[:50]] for k in keys], ["Key Name", "Value Mapping Data"]

            elif self.db_name == "ClickHouse":
                from clickhouse_driver import Client
                client = Client(host=VM_IP, port=int(self.config['port']), connect_timeout=2)
                rows, headers = client.execute('SELECT id, timestamp, event FROM events LIMIT 10'), ["ID", "Timestamp", "Event"]
            ...
            self.signals.data_fetched.emit(["Storage Status"], [[f"Offline / Refused Connect. Reason: {str(fetch_fault)}"]])
